<template>
  <div class="bg-gray-900 py-10 my-16 max-w-7xl mx-auto">
    <h2 class="px-4 text-3xl font-normal leading-7 text-white sm:px-6 lg:px-8">Leadersboard</h2>
        ...
        <tr v-for="item in barObj" :key="item.id">
          <td class="py-4 pl-4 pr-8 sm:pl-6 lg:pl-8">
            <div class="flex items-center gap-x-4">
              <img src="https://images.unsplash.com/photo-1519244703995-f4e0f30006d5?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="" class="h-8 w-8 rounded-lg bg-gray-800" />
              <div class="truncate text-sm font-medium leading-6 text-white">{{ item.name }}</div>
            </div>
          </td>
          <td class="hidden py-4 pl-0 pr-4 sm:table-cell sm:pr-8">
            <div class="flex gap-x-3">
              <div class="font-mono text-sm leading-6 text-gray-400"> {{ item.email }} </div> 
            </div>
          </td>
          <td class="py-4 pl-0 pr-4  text-sm leading-6 sm:pr-8 lg:pr-20">
            <div class="flex  items-center justify-end gap-x-2 sm:justify-start">
              ...
              <span class="text-gray-400 font-xs">9/10</span>
          ...
          <td class="hidden py-4 pl-0 pr-8 text-sm leading-6 text-gray-400 md:table-cell lg:pr-20">last</td>
